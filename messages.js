...
async function main(event) {
    ...
    //     * block MMS, get non-mms number, idk. https://stackoverflow.com/questions/44787713/disabling-mms-on-twilio
    ...
    //     * end game option, clear everyone's currentRoom
    //     * allow vip to force skip to next round.
    if ("Records" in event) {
        // it's from sns
        ...
        if (trimmedMessage.startsWith("newgame")) {
            let roundLimit =
                parseInt(trimmedMessage.match(/^(.*?)([0-9]+)?$/)[2]) ||
                DEFAULT_ROUND_LIMIT;
...
async function handleRoundLimit(event, player, roundLimit) {
    ...
    if (!room.get("isReady")) {
        if (vip !== player.get("number")) {
            return sendSMS(
                player.get("number"),
                `Only the game leader 👑, ${vip}, can change the round limit.`
            );
        }
        ...
        if (isNaN(roundLimit) || roundLimit < 1 || roundLimit > MAX_ROUNDS) {
            ...
            await sendSMS(
                vip,
                `Oops! 😝 You must specify a round limit between 1 and ${MAX_ROUNDS}.\n` +
            ...
            await dynamoClient.updateRoom({
                ...
                roundLimit: setRoundLimit
            });
            return sendSMS(
                vip,
                `Successfully updated round limit to ${setRoundLimit}. ✅`
            );
        }
    }
...
async function handleCharLimit(event, player, charLimit) {
    ...
    if (!room.get("isReady")) {
        if (vip !== player.get("number")) {
            return sendSMS(
                player.get("number"),
                `Only the game leader 👑, ${vip}, can change the response character limit.`
            );
        }
        ...
        if (isNaN(charLimit) || charLimit < 1 || charLimit > MAX_CHAR_LIMIT) {
            ...
            await sendSMS(
                vip,
                `Oops! 😝 You must specify a character limit between 1 and ${MAX_CHAR_LIMIT}.\n` +
            ...
            return sendSMS(
                vip,
                `Successfully updated response character limit to ${setCharLimit} ✅`
            );
        }
    }
    // else if (vip === player.get('number')) {
...
async function welcome(From) {
    ...
    return sendSMS(
        From,
        `😀Welcome to Story Builder! Let's play! 🕹 Respond with ✨'New Game' ✨to get started. Or join an existing ` +
...
async function voice(record) {
    ...
    const message = `All done! Thanks for waiting 😊 Your story turned out to be: ${text.trim()}. Here is the audio! ${url}`;
    console.log("sending final text to ", story.get("starter"));
    await sendSMS(story.get("starter"), message);
    await sendSMS(
        story.get("starter"),
        `Come back soon!\n\nI want your feedback and your money! \n💰✉️ mwilde345@gmail.com\n- Matt Wilde, creator`
    );
...
async function newGame(vip, roomCode, player, roundLimit) {
    ...
    if (
        ...
        currentRoom.toUpperCase() !== roomCode.toUpperCase()
    ) {
        return sendSMS(
            vip,
            `🤔You are already in game: ${player.get(
                "currentRoom"
    ...
    await sendSMS(
        vip,
        `😎Welcome to a new game of Story Builder!\n` +
    ...
    return sendSMS(
        vip,
        `When all players have joined, respond 'Ready'. This will start the game and lock in the rules.`
    );
...
async function ready(event) {
    ...
    if (vip !== roomVip && !isReady) {
        return sendSMS(
            vip,
            `😕 You did not start the game ${currentRoom}. The person who started the game ` +
...
async function response(event) {
    ...
    if (!message || !message.length || message.replace(" ", "").length === 0) {
        return sendSMS(
            player.get("number"),
            `🤔 There was no text in that message, try again with some actual text. (No pictures!)`
        );
    }
    if (message.length > charLimit) {
        return sendSMS(
            player.get("number"),
            `😋Oops! Your response was longer than the rules specified. You have ${Body.length} ` +
...
async function setName(name, From) {
    ...
    await dynamoClient.updateRoom({
        roomCode: room.get("roomCode").toUpperCase(),
        startTime: room.get("startTime"),
    ...
    if (From !== room.get("vip")) {
        await sendSMS(
            room.get("vip"),
            `${From} changed their name from ${oldPlayerName} to ${name}.`
        );
    }
...
async function join(roomCode, From) {
    ...
    if (currentRoom && currentRoom.toUpperCase() !== roomCode.toUpperCase()) {
        return sendSMS(
            From,
            `🙃 You are still in a game: ${player.get("currentRoom")}. ` +
    ...
    if (players.find(player => player.number === From) != undefined) {
        ...
        return sendSMS(
            From,
            `😊 Hello ${player.get("name")}, you have already joined this game!`
        );
    } else if (room.get("isReady")) {
        return sendSMS(
            From,
            `Sorry, this game has already started. 😡 Tell ${room.get(
                "vip"
...
async function sendSQS(data, isJoin) {
    ...
    return new Promise((res, rej) => {
        sqs.sendMessage({
            ...
            // MessageDeduplicationId: `m-${data.roomCode}-${data.From}-${data.Body}`,
